...
func main() {
	...
	prefix := fs.String("prefix", "", "A relative path to append to all paths the server will listen for requests on.")
	...
	if *enable_www {
		...
		*path_static = filepath.Join(*prefix, *path_static)
		...
		*path_www = filepath.Join(*prefix, *path_www)
		mux.Handle(*path_www, http.StripPrefix(*prefix, application_handler))
	}

	if *enable_parse_api {

		*path_parse_api = filepath.Join(*prefix, *path_parse_api)
		...
		mux.Handle(*path_parse_api, http.StripPrefix(*prefix, api_parse_handler))
	}

	if *enable_valid_api {

		*path_valid_api = filepath.Join(*prefix, *path_valid_api)
		...
		mux.Handle(*path_valid_api, http.StripPrefix(*prefix, api_valid_handler))
	}

	if *enable_matches_api {

		*path_matches_api = filepath.Join(*prefix, *path_matches_api)
		...
		mux.Handle(*path_matches_api, http.StripPrefix(*prefix, api_matches_handler))
	}

	if *enable_edtf_string_api {

		*path_edtf_string_api = filepath.Join(*prefix, *path_edtf_string_api)
		...
		mux.Handle(*path_edtf_string_api, http.StripPrefix(*prefix, api_string_handler))
	}

	if *enable_edtf_date_api {

		*path_edtf_date_api = filepath.Join(*prefix, *path_edtf_date_api)
		...
		mux.Handle(*path_edtf_date_api, http.StripPrefix(*prefix, api_date_handler))
	}
